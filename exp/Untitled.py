...
for i in range(len(testimage)):
    tmp.append(testimage[i]^testout[i])
#print(tmp)
...
def decrypt(pixiv,x0,m,n):
    ...
    for i in range(m):
        for j in range(n):
            index = n*i+j
            ch = int(bins[2*index:2*index+2],2)
            cipher[index] = (pixiv[index]^key[ch])&0xff
            print(ch, end=' ')
    return cipher
...
t=(res+decrypt(flagout[len(res):],x1,24-16,16))
print([chr(i)for i in t])
